        ...
        @foreach($addresses as $address)
            <div class="flex mt-4">
                <div class="flex-1 space-x-2">
                    <strong>{{ $address->full_name }}</strong> <span class="border-l"></span> <span class="text-gray-600 text-sm">{{ $address->phone_number }}</span>
                </div>
                <div class="flex space-x-2">
                    ...
                    <form method="POST" action="{{ route('address.destroy', $address) }}">
                        @csrf
                        @method('delete')
                        <x-primary-link href="{{ route('address.destroy', $address) }}" onclick="event.preventDefault(); this.closest('form').submit();">
                            Hapus
                        </x-primary-link>
                    </form>
                </div>
